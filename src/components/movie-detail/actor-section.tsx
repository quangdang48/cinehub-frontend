...
export const ActorSection: React.FC<ActorSectionProps> = ({ actors }) => {
  return (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
            {actors.map(actor => (
                <div key={actor.id} className="group text-center">
                    ...
                    <img src={actor.photoUrl || 'https://randomuser.me/api/portraits/women/2.jpg'} alt={actor.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    </div>
                    <p className="text-white text-sm font-bold group-hover:text-yellow-400 transition-colors">{actor.name}</p>
                    <p className="text-gray-500 text-xs mt-1">{actor.bio}</p>
                </div>
            ))}
        </div>
    </div>
