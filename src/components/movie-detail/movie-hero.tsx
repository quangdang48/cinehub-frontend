...
export const MovieHero: React.FC<MovieHeroProps> = ({ film, trendingNumber }) => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full min-h-[90vh] flex items-end pb-12 overflow-hidden group"
    >
      {/* Background Layers */}
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10 flex flex-col md:flex-row gap-10 items-end">
        ...
        <div className="flex-1 text-white pb-2">
          <div className="flex items-center gap-3 mb-4 animate-fade-in-up">
            ...
            <span className="bg-white/10 backdrop-blur-md text-white text-xs font-bold px-3 py-1 rounded border border-white/10">
              {film.ageLimit}
            </span>
            ...
            {film.type === "SERIES" && (
              <span className="bg-blue-600/20 text-blue-400 text-xs font-bold px-2 py-1 rounded border border-blue-500/30">
                Series
              </span>
            )}
          </div>
          ...
          <h2 className="text-2xl text-yellow-500/90 font-medium mb-6 tracking-wide">
            {film.originalTitle}
          </h2>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {film.genres.map((genre) => (
              <span
                key={genre.id}
                className="bg-white/5 border border-white/10 text-gray-300 text-xs px-3 py-1 rounded-full hover:bg-white/10 hover:text-yellow-400 hover:border-yellow-400/50 cursor-pointer transition-all"
              >
                {genre.name}
              </span>
            ))}
