...
export const RankingCard: React.FC<RankingCardProps> = ({ movie, index, hoveredId, onEnter, onLeave }) => {
  ...
  return (
    <div 
      ...
      `}
    >
      <div className={`relative aspect-2/3 rounded-xl overflow-hidden shadow-lg transition-all duration-300 ${skewClass} 
      ...
      `}>
        <div className={`w-[120%] h-full -ml-[10%] ${unSkewClass} relative`}>
          ...
          <div className="absolute bottom-2 left-2 right-2 flex gap-1 justify-center">
            ...
            <span key='idx' className={`bg-slate-600 text-[10px] font-bold text-white px-1.5 py-0.5 rounded shadow-sm`}>
                T
            </span>
          </div>
        </div>
      ...
      <div className="flex items-start gap-3 mt-4 px-2">
        <span className="text-6xl font-black italic text-[#eab308] leading-none drop-shadow-xl z-20" style={{ fontFamily: 'serif' }}>
          {index + 1}
        </span>
