...
export const WideCard: React.FC<WideCardProps> = ({ movie, onEnter, onLeave }) => {
    ...
    return (
      ...
      >
        ...
        <div className="relative aspect-video rounded-xl overflow-hidden shadow-lg group-hover:shadow-yellow-500/20 transition-all duration-300 group-hover:scale-[1.02] bg-slate-800">
             <img 
                src={movie.posters.find(p => p.type === "backdrop")?.url || "/placeholder.svg"} 
                alt={movie.title} 
                className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                onError={(e) => e.currentTarget.src = "https://via.placeholder.com/400x225/1e293b/ffffff?text=BACKDROP"}
            />
        ...
        <div className="flex gap-4 px-3 -mt-16 md:-mt-20 relative z-10">
            <div className="relative w-20 md:w-[100px] aspect-2/3 flex-none rounded-lg overflow-hidden shadow-lg ring-1 ring-white/10 group-hover:ring-yellow-500/50 transition-all duration-300 group-hover:-translate-y-2">
                 <img 
                    src={movie.posters.find(p => p.type === "default")?.url || "/placeholder.svg"} 
                    alt="poster" 
                    className="w-full h-full object-cover"
                    onError={(e) => e.currentTarget.src = "https://via.placeholder.com/100x150/1e293b/ffffff?text=POSTER"}
                />
                 <div className="absolute top-1 right-1">
                    ...
                    <span key="idx" className={`bg-slate-500 text-[9.2px] font-bold text-white px-1 py-0.5 rounded shadow-sm opacity-90 block mb-1 text-center`}>
                        T
                    </span>
                </div>
            </div>
            <div className="flex flex-col justify-end pb-1 overflow-hidden">
                ...
                <div className="flex items-center flex-wrap gap-2 text-[10px] md:text-xs text-gray-300 font-medium">
                     <span className="text-white font-bold">{movie.genres[0]?.name}</span>
