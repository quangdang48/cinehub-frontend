...
import { MessageSquare, Star, ChevronDown, AlertCircle, TrendingUp, Loader2 } from 'lucide-react';
...
interface WatchCommentSectionProps {
  filmId: string;
  averageRating?: number;
}

export const WatchCommentSection: React.FC<WatchCommentSectionProps> = ({
  filmId,
  averageRating = 0,
}) => {
  ...
  const handleSubmitComment = useCallback(async (content: string) => {
    ...
    try {
      ...
      const response = await CommentsService.commentControllerCreateV1({ requestBody: createDto });
  ...
  const handleReplyComment = useCallback(async (parentId: string, content: string): Promise<CommentDto | null> => {
    if (!signedIn) {
  ...
  const handleLikeComment = useCallback(async (id: string) => {
    if (!signedIn) {
  ...
  const handleSubmitReview = useCallback(async (content: string, rating: number) => {
    ...
    try {
      ...
      const response = await ReviewsService.reviewControllerCreateV1({ requestBody: createDto });
  ...
  const handleLikeReview = useCallback(async (id: string) => {
    if (!signedIn) {
  ...
  const handleSubmitReviewComment = useCallback(async (reviewId: string, content: string): Promise<void> => {
    if (!signedIn) {
      setError('Vui lòng đăng nhập để bình luận');
      return;
    }
    ...
    await CommentsService.commentControllerCreateV1({ requestBody: createDto });
  ...
  const handleReplyToReviewComment = useCallback(async (parentId: string, content: string): Promise<CommentDto | null> => {
    if (!signedIn) {
      setError('Vui lòng đăng nhập để trả lời bình luận');
      return null;
    }
  ...
  return (
    <div className="bg-[#0f172a]/50 backdrop-blur-sm rounded-3xl p-6 lg:p-8 border border-white/5 shadow-2xl animate-fade-in">
              ...
              <div key={star} className="flex items-center gap-2 text-xs">
                ...
                <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${totalReviews > 0 ? ((ratingDistribution[star] || 0) / totalReviews) * 100 : 0}%` }}
                  />
                </div>
                <span className="text-gray-500 w-4">{ratingDistribution[star] || 0}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-6">
        {isLoading && (activeTab === 'comments' ? comments.length === 0 : reviews.length === 0) ? (
      ...
      {((activeTab === 'comments' && comments.length < totalComments) ||
        ...
        <div className="mt-8 text-center">
          <button
            onClick={activeTab === 'comments' ? handleLoadMoreComments : handleLoadMoreReviews}
            disabled={isLoading}
          ...
          >
            {isLoading ? (
              ...
              <>
                <ChevronDown size={18} />
                Xem thêm ({activeTab === 'comments' 
                  ? totalComments - comments.length 
                  : totalReviews - reviews.length} còn lại)
              </>
            )}
          </button>
        </div>
